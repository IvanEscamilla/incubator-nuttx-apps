...
static inline void lesp_clear_read_ans(void) { g_lesp_state.and = LESP_NONE; }
                ...
                if (rxlen != 0)
                {
                    ...
                    else
                    {
                        if (worker->buf[0] != '\0')
                        {
                            ...
                            usleep(100); /* leave time of aplicative to read
                                            buffer */
                            pthread_mutex_lock(&(worker->mutex));
                        }
...
static inline int lesp_create_worker(int priority)
{
    ...
    else
    {
        ...
        ret =
            pthread_create(&g_lesp_state.worker.thread,
 ...
 *
 * Input Parameters:
 *   mode : LESP_MODE_AP or lesp_eMODE_STATION
 *
 * Output Parameters:
 *   ip     : ip of interface
 ...
 *   Warning: use lesp_eMODE_STATION or LESP_MODE_AP.
 *
 * Input Parameters:
                 ...
                 in_addr_t gateway)
{
    ...
    if (ret >= 0)
    {
        ret = lesp_ask_ans_ok(
            LESP_TIMEOUT_MS,
            "AT+CIP%s_CUR=\"%d.%d.%d.%d\","
            "\"%d.%d.%d.%d\",\"%d.%d.%d.%d\"\r\n",
            (mode == lesp_eMODE_STATION) ? "STA" : "AP", *((uint8_t *)&(ip) + 0),
            *((uint8_t *)&(ip) + 1), *((uint8_t *)&(ip) + 2),
            *((uint8_t *)&(ip) + 3), *((uint8_t *)&(gateway) + 0),
            *((uint8_t *)&(gateway) + 1), *((uint8_t *)&(gateway) + 2),
            *((uint8_t *)&(gateway) + 3), *((uint8_t *)&(mask) + 0),
...
int lesp_setsockopt(int sockfd, int level, int option, FAR const void *value,
                    socklen_t value_len)
{
    int ret = 0;
    lesp_socket_t *sock;
